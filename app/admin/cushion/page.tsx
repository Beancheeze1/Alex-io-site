...
type RecInput = {
  weight_lbf: number; area_in2: number; thickness_in: number;
  fragility_g: number; drop_in: number; overlay_count?: number;
};
...
export default function CushionAdminPage() {
  ...
  async function suggestThickness() {
    // compute (if not already)
    if (!data) await calc();
    const d = data || (await (await fetch("/api/cushion/recommend", {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form)
    })).json());
    if (!d?.thickness_recommendation?.overall_min) return;
    const t = d.thickness_recommendation.overall_min.min_thickness_in;
    setForm(f => ({ ...f, thickness_in: t }));
    // re-run plot with suggested thickness
    setTimeout(calc, 0);
  }

  async function refreshSourceInfo() {
    try {
      const r = await fetch("/api/cushion/upload", { method: "GET" });
      const j = await r.json();
      setSourceInfo(j);
    } catch {}
  }
  useEffect(() => { refreshSourceInfo(); }, []);
  ...
  async function onUploadFile(ev: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await fetch("/api/cushion/upload", { method: "POST", body: fd });
      const j = await res.json();
      if (!res.ok) throw new Error(j?.error || "Upload failed");
      setUploadMsg(`Uploaded ${j.items} curves → active source: ${j.active_source}`);
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-8">
      ...
      <form onSubmit={onCalc} className="bg-white rounded-2xl shadow p-4 grid grid-cols-1 md:grid-cols-12 gap-3">
        ...
        <div className="md:col-span-12 flex flex-wrap gap-3">
          <button className="bg-black text-white rounded-lg px-4 py-2 disabled:opacity-40" disabled={busy} type="submit">
            {busy ? "Calculating…" : "Recommend"}
          </button>
          <button type="button" onClick={suggestThickness}
                  className="bg-emerald-600 text-white rounded-lg px-4 py-2 disabled:opacity-40"
                  disabled={busy}>
            Suggest Thickness & Re-plot
          </button>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow p-4 space-y-3">
        <div className="text-sm">
          <b>Active curve source:</b>{" "}
          {sourceInfo?.active_source ?? "unknown"}{" "}
          {sourceInfo?.stats ? (
            <span className="text-gray-600">({sourceInfo.stats.foams} foams, {sourceInfo.stats.series} series)</span>
          ) : null}
        </div>
        <div className="flex items-center gap-3 text-sm">
          <input type="file" accept="application/json,.json" onChange={onUploadFile} />
          <span className="text-gray-500">(Upload a <code>cushion_curves.json</code> — switches source to /tmp)</span>
        </div>
