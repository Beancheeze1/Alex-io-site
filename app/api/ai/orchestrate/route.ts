...
export async function POST(req: NextRequest) {
  try {
    ...
    // get quote html + structured data
    ...
    const quotePayload = qRes.ok ? await qRes.json() : null;

    const html = quotePayload?.html || `
      <div style="font-family:Segoe UI,Arial,Helvetica,sans-serif;font-size:14px;line-height:1.4;color:#111">
        ...
        <p>To lock in pricing, could you confirm (or attach a sketch):<br/>• final outside dimensions (L × W × H)<br/>• quantity<br/>• foam density (e.g., 1.7 pcf)<br/>• thickness under the part<br/>• units (in or mm)</p>
    ...
    // live: hand off to Graph send (threaded reply)
