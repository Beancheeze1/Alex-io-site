...
export async function POST(req: NextRequest) {
  try {
    ...
    // If we’re missing email or text/subject, ask our lookup to hydrate them
    ...
    if (!text) text = ""; // AI can still handle with follow-ups

    // Build AI payload
    const aiTask =
      process.env.AI_TASK ||
      "Reply like a helpful estimator and move toward quoting.";
    const aiHints =
      (process.env.AI_HINTS &&
        process.env.AI_HINTS.split("|").map((s) => s.trim()).filter(Boolean)) ||
      ["<~120 words", "Ask 2 crisp questions", "Foam insert: density + thickness under part"];
    ...
    const orchestrateBody = {
      ...
      subject,
      text,
      dryRun: dryRun || !replyEnabled, // force dryRun if replies disabled
      ai: {
        task: aiTask,
        hints: aiHints,
      },
    ...
    // Only stamp idempotency/cooldown when dryRun=false and we “sent”
