...
export async function POST(req: NextRequest) {
  try {
    ...
    // Micro-throttle: ignore fast duplicates within a few seconds
    ...
    let vars: Record<string, string> = {}; // {{firstName}}, etc.
    ...
    // Quote token + link (stored in KV with TTL)
    const quoteId =
    ...
    // B2: optional brand wrapper
    const html = shouldWrap() ? wrapHtml(innerHtml) : innerHtml;

    // B3: plain-text fallback derived from INNER HTML (not the wrapper)
    ...
    if (dryRun || !replyEnabled) {
      return NextResponse.json({
        ...
        quoteLink,
        wrapped: shouldWrap(),
        template: { subject, htmlPreview: html.slice(0, 180), textPreview: text.slice(0, 180) },
      });
    }

    // Send via internal msgraph route (that route adds loop header; no In-Reply-To/References)
    const sendRes = await postJson(SEND, { to: toEmail, subject, html, text });
    if (!sendRes.ok) {
    ...
    // Success → mark idempotent & cooldown
    if (rawMessageId)
      await kv.set(
