...
export async function GET(req: Request) {
  try {
    ...
    const res = await fetch(`${base}/api/redis-check`, { cache: "no-store" });
    const text = await res.text();
    try {
      const json = JSON.parse(text);
      return NextResponse.json(json, { status: res.status });
    } catch {
      return NextResponse.json({ ok: false, note: "redis-check returned non-JSON", status: res.status, body: text }, { status: 200 });
    }
  } catch (err: any) {
