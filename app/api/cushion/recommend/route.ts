...
//
// Honesty constraint: the cushion_curves table currently has exactly ONE
// digitized foam thickness per material curve (2in, see cushion_curves.
// thickness_in). So "minimum thickness" here means "the thickness of the
// best real curve match that meets your fragility target" -- NOT a true
// swept minimum across multiple tested thicknesses, because that data does
// not exist. If no material meets the target at its tested thickness,
// min_thickness_in comes back null rather than a guessed number.
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const thickness_recommendation = best
      ? {
          overall_min: {
            min_thickness_in: best.curve.thickness_in,
            basis: "tested_curve_match" as const,
            material_id: best.material_id,
            material_name: best.name,
            g_at_operating_psi: best.g_at_operating_psi,
            note:
              "min_thickness_in is the thickness of the closest real cushion-curve match that meets your fragility target at this static load -- not a swept minimum across multiple tested thicknesses (only one thickness is digitized per material curve on file).",
          },
        }
      : {
          overall_min: {
            ...
            note:
              "No cataloged material's tested curve meets your fragility target at this static load and the single tested thickness on file. Provide height_in explicitly rather than relying on an unsupported guess.",
          },
        };
