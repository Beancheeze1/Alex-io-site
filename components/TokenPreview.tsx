...
import SkiveCard from "./SkiveCard";

type SkiveApplied = { needsSkive?: boolean; skiveChargeEach?: number };
type Props = { qty?: number; applied?: SkiveApplied };

export default function TokenPreview({ qty, applied }: Props) {
  const swatches = [
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-md">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button className="rounded-lg bg-neutral-200 px-4 py-2 text-neutral-800 hover:bg-neutral-300">
            Secondary
          </button>
          <button className="rounded-lg bg-success px-4 py-2 text-white hover:opacity-90">
            Success
          </button>
          <button className="rounded-lg bg-warning px-4 py-2 text-white hover:opacity-90">
            Warning
          </button>
          <button className="rounded-lg bg-danger px-4 py-2 text-white hover:opacity-90">
            Danger
          </button>
        </div>
      </section>
      ...
      {/* Skiving Card (mount point) */}
      <section className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-md">
        <h2 className="mb-4 text-lg font-semibold text-neutral-900">Skiving</h2>
        <SkiveCard qty={qty ?? 0} applied={applied} />
      </section>
    </div>
  );
}
