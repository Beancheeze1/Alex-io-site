...
export async function kvGet(key) {
  ...
  if (!rec) return null;
  if (rec.exp && rec.exp < nowSec()) { mem.delete(key); return null; }
  return rec.val ?? null;
}
export async function kvSet(key, val, ttlSec) {
  if (UPSTASH_URL && UPSTASH_TOKEN) {
    const r = await fetch(`${UPSTASH_URL}/set/${encodeURIComponent(key)}`, {
      ...
      body: JSON.stringify(ttlSec ? { value: val, ex: ttlSec } : { value: val })
    });
    return r.ok;
  }
...
export async function kvDel(key) {
  if (UPSTASH_URL && UPSTASH_TOKEN) {
    await fetch(`${UPSTASH_URL}/del/${encodeURIComponent(key)}`, { method: "POST", headers: { Authorization: `Bearer ${UPSTASH_TOKEN}` } });
    return true;
  }
  mem.delete(key);
  return true;
