...
export const PriceRule = z.object({
  ...
  // v4 requires valueType: use (z.string(), z.any()) to allow arbitrary JSON
  formula: z.union([z.string(), z.record(z.string(), z.any())]).optional(),
});
...
export const PriceBook = z.object({
  ...
  created_at: z.string().datetime().optional(), // ok in v4
  notes: z.string().optional(),
  tables: z.object({
